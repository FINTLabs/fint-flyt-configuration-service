...
@Component
public class DokumentStatusResourceEntityConsumer extends EntityConsumer<DokumentStatusResource> {

    protected DokumentStatusResourceEntityConsumer(ObjectMapper objectMapper, FintCacheManager fintCacheManager) {
        super(objectMapper, fintCacheManager);
    }

    @Override
    protected String getResourceReference() {
        return "arkiv.kodeverk.dokumentstatus";
    }

    @Override
    protected Class<DokumentStatusResource> getResourceClass() {
        return DokumentStatusResource.class;
    }

    @Override
    protected List<String> getKeys(DokumentStatusResource resource) {
    ...
    @Bean
    String dokumentStatusResourceEntityTopicName(TopicNameService topicNameService) {
        return topicNameService.generateEntityTopicName(DomainContext.SKJEMA, this.getResourceReference());
    }

    @Override
    @KafkaListener(topics = "#{dokumentStatusResourceEntityTopicName}")
